...
int info_server_create(info_server_t *self, const char *servicename){
    ...
    status = getaddrinfo(0 /* ANY */, self->servicename, &(self->hints), &(self->results));
...
int info_server_establish_connection(info_server_t *self){
    ...
    for (addr_ptr = (self->results); addr_ptr != NULL; addr_ptr = addr_ptr->ai_next) {
        if (socket_create(&(self->blsocket)) == -1) {
            ...
            if (socket_bind(&(self->blsocket), addr_ptr->ai_addr, addr_ptr->ai_addrlen) != -1) {
                break;
            }
    ...
    if (socket_accept(&(self->blsocket), addr_ptr->ai_addr, &(addr_ptr->ai_addrlen), &(self->peersocket)) == -1) {
        printf("Error: %s\n", strerror(errno));
    }
}
